...
void loop()
{
    ...
    if(x>240) moode=!moode;
    if(moode==0)
    {
        if(calendarStarted==0) startCalendar();
        if (y>0) 
        {
            if(pres==1)
            {
    ...
    if(moode==1)
    { 
        if(calcStarted==0) startCalc();
        if (pos.x>0) 
        {
            if(pos.y>0 && pos.y<40) resetCalc();
            if(pres==1) for(int i=0;i<n2;i++) for(int j=0;j<n2;j++)
            {
                if(pos.x>posX2[j] && pos.x<posX2[j]+boxSize2 && pos.y>posX2[i]+22 && pos.y<posX2[i]+boxSize2+22 )
                {
                    ...
                    if(buttons2[j][i]=='+' || buttons2[j][i]=='-' || buttons2[j][i]=='*' || buttons2[j][i]=='/' )
                    {
                        ...
                        if(numbers2[0]==0) numbers2[0] = math2.toFloat();
                        math2="";
                    ...
                    if(buttons2[j][i]=='=' )
                    {
                         ...
                         if(operation2=='+') numbers2[2]=numbers2[0]+numbers2[1];
                         if(operation2=='-') numbers2[2]=numbers2[0]-numbers2[1];
                         if(operation2=='*') numbers2[2]=numbers2[0]*numbers2[1];
                         if(operation2=='/') numbers2[2]=numbers2[0]/numbers2[1];
                         numbers2[0]=numbers2[2];
                         int check2=numbers2[2]*100.00;
                         if(check2%100==0)   math2=String(int(numbers2[2]));
                         else math2=String(numbers2[2]);
                         M5.Lcd.fillRoundRect(posX2[0],3,188,40,4,dark);
                    }
                    bool decimal=false;
                    int check2=math2.toFloat()*100.00;
                    if(check2%100==0) decimal=false;
                    else decimal=true;

                    M5.Lcd.setFreeFont(&DSEG7_Classic_Bold_30);
                    M5.Lcd.setTextColor(light,dark); //.............................................
                    if(decimal==true) M5.Lcd.drawString(math2,30,8);
                    else M5.Lcd.drawString(math2,30,8);
...
void resetCalc()
{
    for(int i=0;i<3;i++) numbers2[i]=0;
    math2="";
    M5.Lcd.fillRoundRect(posX2[0],3,188,40,4,dark);
}
...
void startCalendar()
{
    ...
    for(int i=0;i<n;i++) posX[i]=(start+(i*boxSize)+(space*i));
    fill(0);
...
void startCalc()
{
    ...
    M5.Lcd.setTextFont(4);
    for(int i=0;i<n2;i++) posX2[i]=(start2+(i*boxSize2)+(space2*i));
    M5.Lcd.fillRoundRect(posX2[0],3,188,40,4,dark);

    for(int i=0;i<n2;i++) for(int j=0;j<n2;j++)
    { 
        if(j<3 && i <3 ) M5.Lcd.fillRoundRect(posX2[i],posX2[j]+22,boxSize2,boxSize2,4,dark);
        else M5.Lcd.fillRoundRect(posX2[i],posX2[j]+22,boxSize2,boxSize2,4,darkOrange);
